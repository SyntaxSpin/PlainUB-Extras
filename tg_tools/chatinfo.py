...
async def format_chat_info(chat: Chat, is_full: bool) -> tuple[str, str | None]:
    if is_full:
        ...
        info_lines.extend([f"• <b>Title:</b> {safe_escape(chat.title)}", f"• <b>ID:</b> <code>{chat.id}</code>"])
        ...
        if chat.username: info_lines.extend([f"• <b>Username:</b> @{chat.username}", f"• <b>Permalink:</b> <a href='https://t.me/{chat.username}'>{chat.username}</a>"])
        if chat.description:
            desc = chat.description
            info_lines.append(f"• <b>Description:</b> {safe_escape(desc[:200] + '...' if len(desc) > 200 else desc)}")
        if chat.members_count: info_lines.append(f"• <b>Members:</b> {chat.members_count}")
        ...
        if chat.dc_id: info_lines.append(f"• <b>Data Center:</b> {chat.dc_id}")
        if chat.linked_chat: info_lines.append(f"• <b>Linked Chat ID:</b> <code>{chat.linked_chat.id}</code>")
    else:
        info_lines = ["<b>Chat info:</b>", f"ID: <code>{chat.id}</code>", f"Title: {safe_escape(chat.title)}"]
        if chat.username: info_lines.append(f"Username: @{chat.username}")
        if chat.members_count: info_lines.append(f"Members: {chat.members_count}")
        info_lines.append(f"Chat link: <a href='https://t.me/{chat.username or f'c/{chat.id}'}'>link</a>")
...
async def chat_info_handler(bot: BOT, message: Message):
    ...
    try:
        ...
        if target_chat.type == ChatType.PRIVATE:
            return await progress_msg.edit("Use <code>.info</code> for users.", del_in=8)
        ...
        if photo_id:
            ...
            try:
                ...
                if os.path.exists(photo_path): shutil.rmtree(TEMP_CINFO_DIR, ignore_errors=True)
        else:
            await message.reply(final_text, link_preview_options=LinkPreviewOptions(is_disabled=True))
    except Exception as e:
        await progress_msg.edit(f"<b>Error:</b> Could not find chat.\n<code>{safe_escape(str(e))}</code>", del_in=10)
