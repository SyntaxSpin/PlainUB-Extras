...
async def sync_rotate_video(input_path: str, rotations: int) -> str:
    """Synchronously rotates a video by applying the transpose filter N times."""
    base, ext = os.path.splitext(os.path.basename(input_path))
    output_path = os.path.join(TEMP_DIR, f"{base}_rotated{ext}")
...
async def rotate_handler(bot: BOT, message: Message):
    """
    CMD: ROTATE
    INFO: Rotates the replied image or video.
    USAGE:
    ...
    if not is_media:
        return await message.edit("Please reply to an image or video to rotate it.", del_in=ERROR_VISIBLE_DURATION)
    ...
    try:
        ...
        if is_image:
            ...
            modified_path = await sync_rotate_video(original_path, rotations)
