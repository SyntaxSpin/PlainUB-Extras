...
def sync_upscale_image(input_path: str, scale_factor: int = 2) -> tuple[str, int, int]:
    """Synchronously upscales an image."""
...
async def sync_upscale_video_or_gif(input_path: str, scale_factor: int = 2) -> tuple[str, int, int]:
    """Synchronously upscales a video or GIF using FFmpeg."""
    base, ext = os.path.splitext(os.path.basename(input_path))
    output_path = os.path.join(TEMP_DIR, f"{base}_upscaled{ext}")
    
    # Use ffprobe to get original dimensions to calculate the new size for the caption
    ...
    # The scale filter iw*2:ih*2 dynamically doubles the input width and height
...
async def upscale_handler(bot: BOT, message: Message):
    """
    CMD: UPSCALE
    INFO: Upscales the replied image, video, or GIF to 2x its original size.
    USAGE:
        .upscale
    """
    ...
    if not is_media:
        return await message.edit("Please reply to an image, video, or GIF to upscale it.", del_in=ERROR_VISIBLE_DURATION)
    ...
    try:
        ...
        if is_image:
            ...
            modified_path, new_width, new_height = await sync_upscale_video_or_gif(original_path)
